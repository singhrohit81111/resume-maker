...
const login = asyncHandler(async (req, res, next) => { 
    ...
    const{accessToken,refreshToken}=await tokenService.generateAccessAndRefreshToken(user._id);
    const currentUser=await authService.getCurrentUser(user._id);
    res.status(200).cookie("accessToken",accessToken,OPTIONS).cookie("refreshToken",refreshToken,OPTIONS).json(currentUser)
});
